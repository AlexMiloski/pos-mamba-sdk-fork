...
module.exports = {
  ...
  builder: (yargs) => {
    ...
    if (typeof PKG.mamba === 'undefined') {
      console.error(chalk.red(`No "mamba" property found at "${PKG.rootDir}" Is this an mamba app directory?"`));
      process.exit(1);
    }

    /** Add the node_modules/.bin of the current app to the PATH env variable */
    process.env.PATH += (process.platform === 'win32' ? ';' : ':') + fromCwd('node_modules', '.bin');
