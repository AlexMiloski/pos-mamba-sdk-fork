...
export function setup(Payment) {
   ...
   */
  Payment.getCardHolderName = () => (!Payment.failedPaying() ? '' : Registry.get().$Payment.cardHolderName);
   ...
   */
  Payment.getAmountAuthorized = () => (Payment.failedPaying() ? 0 : Registry.get().$Payment.authorizedAmount);
   ...
   */
  Payment.getAuthorizationCode = () => (!Payment.failedPaying() ? '' : Registry.get().$Payment.authCode);
   ...
   */
  Payment.getInstallmentCount = () => (!Payment.failedPaying() ? 0 : Registry.get().$Payment.installmentCount);
