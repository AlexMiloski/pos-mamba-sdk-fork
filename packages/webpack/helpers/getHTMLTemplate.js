...
const enIp = () => {
  ...
  while (keys.length) {
    ...
    if (ifname === 'en0') {
      const found = ifaces[ifname].find(({ family, internal }) => family === 'IPv4' && internal === false);
    ...
    if (weinreIp) {
      ...
      process.on('exit', () =>
        console.log(
          ...
          chalk.yellow('weinre --boundHost=-all- --httpPort=9000 --readTimeout=8 --deathTimeout=60 \n\n'),
        ),
      );
    }
  }
